...
const DropDown = ({disabled,setFunction}) => {
    ...
    return(
        <div className={`flex-col relative text-center  shadow-md h-12 bg-White rounded-md py-3 w-40  justify-between items-center ${disabled?'opacity-50':null}`}>
            <button className="w-full" disabled={disabled} onClick={() => setIsOpen(!isOpen)}>{choice}</button>
            {isOpen?
                    <ul className="rounded-lg mt-6 relative bg-White w-full shadow-md">
                        <li className="py-1 shadow-sm cursor-pointer hover:bg-DarkGray" value='africa' onClick={({target}) => handleChoice(target.innerHTML)} >All region</li>
                        ...
                        <li className="py-1 shadow-sm cursor-pointer hover:bg-DarkGray" value='oceania' onClick={({target}) => handleChoice(target.innerHTML)}>Oceania</li>
                    </ul>
                    :null
            }
        </div>
